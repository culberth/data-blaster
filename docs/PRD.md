...
| R16 | The ribbon shows a group whose controls follow the selected mode. Only Log (playback speed, log folder) and Message (message type) have one; SOAP and REST leave the slot empty and un-managed. The slot swaps its own content rather than being swapped by the shell, so adding a group still needs no edit to `MainController` | Q7 | Must |
...
| Q7  | ~~R16 (contextual ribbon) is *Should*. In or out for v1?~~ **Answered 2026-08-29: in.** R16 is promoted to *Must* and built. Only Log and Message carry a group — a ribbon is for controls reached for repeatedly, and SOAP's port is set once while REST has no behaviour. This also resolves D11's conditional half: Playback Speed Factor **is** on the ribbon, so it gets the log-scaled slider | — | Closed |
...
| D11 | Playback Speed Factor is a multiplier (`1.0` = real time), edited with a `Spinner<Double>` in Preferences; a log-scaled slider only if R16 puts it in the ribbon — **which Q7 has now done, so both controls exist: the spinner in Preferences for an exact value, a log-scaled slider in the Log ribbon group for scrubbing** | A linear slider; discrete steps (0.25/0.5/1/2/4/8); an integer percent | A spinner takes an exact value without the log/linear problem, which suits a setting configured once rather than scrubbed live. A linear slider strands `1.0` at 9% of its travel (R-2); discrete steps foreclose values before anyone knows which are wanted. Costs the drag-to-adjust feel the inherited slider had |
