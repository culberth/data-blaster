...
import com.culberth.tools.datablaster.ui.ThemeService;
import org.springframework.context.ApplicationContext;
import org.springframework.stereotype.Component;

/**
 * Loads FXML views using Spring as the controller factory, so view controllers can receive dependencies (services,
 * repositories, etc.) from the application context.
 *
 * <p>
 * Controllers resolved this way must be prototype-scoped: the loader binds the returned instance's {@code @FXML} fields
 * to the node tree it just built, so a singleton controller would end up shared between — and pointing at only the
 * newest of — several node trees.
 */
@Component
public class ViewLoader
{
    ...
    /**
     * Applies the app stylesheet and current theme to a node tree that carries its own styling rather than sitting in a
     * scene this class made — an {@code Alert}'s {@code DialogPane} is the only such case today. Without it, an error
     * dialog raised while the dark theme is on arrives in default light colours.
     */
