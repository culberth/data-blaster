...
/**
 * The Log mode view: a read-only account of what Log mode is configured to do, and an honest note
 * that it does not do it yet.
 *
 * <p><strong>Read-only on purpose.</strong> Every value here is editable in the Log ribbon group or
 * the Preferences Log tab. A third editing surface would be a third thing to keep in step; a
 * read-out is not.
 *
 * <p><strong>Bound, not assigned.</strong> Changing a setting anywhere is visible here immediately,
 * without the view being rebuilt — which is also the cheapest end-to-end demonstration that the
 * settings plumbing actually works, since the ribbon, the dialog and this view are three
 * independent readers of one {@link AppState}.
 */
@Component
...
public class LogViewController {
    ...
    private void initialize() {
        ...
        playbackSpeedValue.textProperty().bind(
                Bindings.format("%.2f× real time", appState.playbackSpeedFactorProperty()));
