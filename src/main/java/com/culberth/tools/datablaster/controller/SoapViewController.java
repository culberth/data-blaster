...
/**
 * The SOAP mode view: the configured address, message type, data files and tail number, read-only, and an honest note
 * that nothing uses any of them yet.
 *
 * <p>
 * SOAP has no ribbon group — these are set once for a run — so Preferences is the only place to change them, and here
 * is where you see them without opening that dialog.
 *
 * <p>
 * <strong>Bound, not assigned</strong>, for the reason {@code LogViewController} gives: this view is an independent
 * third reader of {@link AppState}, which is what makes it an end-to-end check that the settings plumbing works rather
