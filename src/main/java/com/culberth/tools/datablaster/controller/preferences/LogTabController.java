 ...
 *
 * <p><strong>Bad entries are rejected where they are made.</strong> Every path that changes the
 * table — Add, and an edit committed in either column — funnels through {@link #apply}, which
 * catches the {@code IllegalArgumentException} that {@link PortTailMapping} and
 * {@link AppState#setPortTailMappings} throw and shows its message beside the controls. Those
 * messages are written to be read by a person, which is why they are shown verbatim rather than
 * translated here. Nothing is deferred to close, and nothing opens a modal to say a port is out of
 * range.
 *
 * <p><strong>Both columns are {@code String} columns, including Port.</strong> An {@code Integer}
 * column would need a converter, and the default one throws on unparsable text from inside the
 * cell's commit — an exception escaping into the table's own event handling, where this class
 * cannot turn it into a message. Parsing the text here keeps every validation failure on one path.
 */
@Component
...
public class LogTabController {
     ...
     * {@code ChangeListener} on an {@code ObservableList} fires only when the list object itself is
     * replaced, which never happens here.
     */
    ...
    private void initialize() {
        ...
        resetButton.disableProperty().bind(
                appState.playbackSpeedFactorProperty()
                        ...
                        .and(Bindings.isEmpty(appState.portTailMappings())));
    }
    ...
    private void initMappingTable() {
        ...
        portColumn.setCellValueFactory(row ->
                new SimpleStringProperty(Integer.toString(row.getValue().port())));
        ...
        portColumn.setOnEditCommit(edit ->
                replace(edit.getRowValue(), edit.getNewValue(), edit.getRowValue().tail()));
        tailColumn.setOnEditCommit(edit ->
                replace(edit.getRowValue(), Integer.toString(edit.getRowValue().port()),
                        edit.getNewValue()));
     ...
     * Parses a typed port.
     *
     * <p>Throws the same {@code IllegalArgumentException} that every other rejection on this path
     * throws, with a message in the same voice, so the caller needs one catch rather than two.
     */
    private static int parsePort(String text) {
        try {
            ...
            throw new IllegalArgumentException(
                    "Port must be a number between " + PortTailMapping.PORT_MIN + " and "
     ...
     *
     * <p>The confirmation is conditional on there being something to destroy. Asking before putting
     * a spinner back would be noise, and a prompt that always appears is a prompt people learn to
     * dismiss without reading — which is exactly the habit that makes the one that matters
     * ineffective.
     */
    @FXML
    ...
    private boolean confirmClearingMappings() {
        ...
        return dialogService.confirm(
                ...
                "This removes " + count + (count == 1 ? " mapping" : " mappings")
