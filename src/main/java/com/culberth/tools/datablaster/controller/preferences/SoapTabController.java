 ...
 *
 * <p>This is the only surface for all four — SOAP has no ribbon group, because these are set once
 * for a run and a ribbon is for controls reached for repeatedly.
 *
 * <p><strong>Bad entries are rejected where they are made</strong>, the same rule the mapping editor
 * follows. The address and the tail both funnel through a commit method that catches the
 * {@code IllegalArgumentException} their validators throw and shows its message verbatim beside the
 * field. Those messages are written to be read by a person, which is why they are not translated
 * here. Nothing is deferred to close, and nothing opens a modal to say an octet is out of range.
 *
 * <p><strong>A {@code TextField} commits on Enter and on losing focus.</strong> Unlike a
 * {@code Spinner} there is no editor to flush, but there is the same failure: typing an address and
 * pressing Close would otherwise discard it. Committing on focus loss as well as on Enter is what
 * makes tabbing to the next field behave the way it looks like it does.
 *
 * <p><strong>A rejected entry leaves the field as the user typed it</strong>, next to the reason.
 * Snapping the text back to the stored value would erase what they were correcting.
 */
@Component
...
public class SoapTabController {
    ...
    /**
     * Kept in step when these are changed elsewhere — today only by this tab's own Reset, but a
     * control that silently disagrees with the state behind it is the defect this whole dialog was
     * reworked to avoid.
     *
     ...
     */
    private final ChangeListener<String> ipListener =
            (observable, old, ip) -> ipField.setText(ip);

    private final ChangeListener<SoapMessageType> messageTypeListener =
            (observable, old, type) -> soapMessageTypeChoice.setValue(type);

    private final ChangeListener<String> tailListener =
            (observable, old, tail) -> tailField.setText(tail == null ? "" : tail);

    public SoapTabController(AppState appState,
                             DataFileChooser dataFileChooser,
    ...
    private void initialize() {
        ...
        resetButton.disableProperty().bind(
                appState.soapIpProperty().isEqualTo(Settings.DEFAULTS.soap().ip())
                        .and(appState.soapMessageTypeProperty()
                                .isEqualTo(Settings.DEFAULTS.soap().type()))
                        .and(appState.soapTailProperty().isNull())
                        .and(Bindings.isEmpty(appState.soapDataFiles())));
    }
    ...
    private void initDataFiles() {
        ...
        removeFileButton.disableProperty().bind(
                dataFileList.getSelectionModel().selectedItemProperty().isNull());
    }
     ...
     *
     * <p>The confirmation is conditional on there being something to discard, for the reason the
     * Log tab's is: a prompt that always appears is a prompt people learn to dismiss without
     * reading, which is exactly the habit that makes the one that matters ineffective.
     */
    @FXML
    ...
    private boolean confirmClearingDataFiles() {
        ...
        return dialogService.confirm(
                "Reset the SOAP settings?",
                "This removes " + count + (count == 1 ? " data file" : " data files")
                        + " from the list and cannot be undone.",
                "Reset");
    }
