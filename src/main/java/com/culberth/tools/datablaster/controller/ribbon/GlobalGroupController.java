 ...
 *
 * <p>This group replaced "Appearance", whose only control was a content-opacity slider. Opacity was
 * a view control rather than a setting and is gone rather than moved; these three are settings, are
 * persisted, and are shown here <em>and</em> in Preferences' General tab. Neither surface owns the
 * value — both read and write {@link AppState}.
 *
 * <p><strong>Every control follows the state as well as writing to it.</strong> The same three
 * settings are editable in Preferences, so a control that read {@code AppState} once would sit
 * there disagreeing with it the moment the dialog changed anything. Setting a control to the value
 * it already holds fires nothing in JavaFX, so the write-back loop closes itself and none of these
 * needs a re-entrancy flag — unlike the Log group's slider, whose log scale means a round trip does
 * not land on the value it started from.
 *
 * <p><strong>This controller is pinned by its own controls.</strong> The subscriptions below are
 * weak, as {@code AppState} requires, but the listeners registered on this group's own spinner and
 * check boxes are lambdas capturing {@code this} — a strong node-to-controller reference that keeps
 * it alive exactly as long as the node tree. {@code ContextualGroupController} has no such control
 * and has to park itself in its host's properties; this one does not.
 */
@Component
...
public class GlobalGroupController {
     ...
     * long as this controller — no longer, or a discarded ribbon would go on reacting.
     */
