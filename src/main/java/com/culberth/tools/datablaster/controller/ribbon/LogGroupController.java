 ...
 *
 * <p>
 * <strong>The slider is in track units, not multipliers.</strong> Its 0–1 range is a position on a logarithmic scale
 * that {@link LogScale} converts to and from — see that class for why a linear track is unusable for a 0.1–10.0
 * multiplier. Nothing outside this controller ever sees a position: {@link AppState} holds the real speed, and the
 * read-out shows it.
 *
 * <p>
 * <strong>Two controls edit this setting and neither owns it.</strong> Preferences has a {@code Spinner} for typing an
 * exact value; this has a slider for scrubbing. Both read and write {@link AppState}, so there is no copy to keep in
 * step.
 */
@Component
...
public class LogGroupController
{
     ...
     *
     * <p>
     * A log scale lands on values like 1.0000000000000002. Rounding here rather than only in the read-out's format
     * matters, because the rounded number is what gets stored — formatting it away would leave the settings file
     * holding noise the UI never showed.
     */
    private static final int SPEED_DECIMALS = 2;

    private static final String NO_FOLDER_SELECTED = "(none selected)";

    private final LogScale speedScale = new LogScale(Settings.PLAYBACK_SPEED_MIN, Settings.PLAYBACK_SPEED_MAX);
     ...
     *
     * <p>
     * Without it the two listeners below form a loop: the slider writes a *rounded* speed, the speed listener converts
     * that back to a position, and the position it computes is not bit-identical to where the thumb is — so the thumb
     * jumps under the cursor mid-drag. The guard makes the relationship one-way for the duration of the write, which is
     * the only moment it needs to be.
     *
     * <p>
     * Declared before {@code speedListener} because that field's initializer reads it, and a field initializer may not
     * refer forward to a later one.
     */
     ...
     *
     * <p>
     * Unlike most controls in this project, which read {@link AppState} once in {@code initialize()} and only write
     * afterwards, this one has to keep following it. Preferences is modal, so the two are never manipulated at the same
     * instant, but closing it after changing the speed would otherwise leave the read-out showing the new value beside
     * a thumb still at the old position — and the next nudge of that thumb would silently throw the new value away.
     *
     * <p>
     * Held strongly here so the weak registration on the singleton {@code AppState} lives exactly as long as this
     * controller, and no longer.
     */
    ...
    public LogGroupController(AppState appState, StageRegistry stageRegistry, LogFolderChooser logFolderChooser)
    {
    ...
    private void initialize()
    {
        ...
        playbackSpeedSlider.valueProperty().addListener((observable, old, position) ->
        {
            ...
            try
            {
                appState.setPlaybackSpeedFactor(speedScale.valueAt(position.doubleValue(), SPEED_DECIMALS));
            }
            finally
            {
        ...
        // the speed.
        playbackSpeedValueLabel.textProperty().bind(Bindings.format("%.2fx", appState.playbackSpeedFactorProperty()));
        ...
        playbackSpeedResetButton.disableProperty()
                .bind(appState.playbackSpeedFactorProperty().isEqualTo(Settings.PLAYBACK_SPEED_DEFAULT, 0.001));
