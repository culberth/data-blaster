 ...
 *
 * <p><strong>Why the mode settings are nested and the global ones are not.</strong> The modes'
 * settings have nothing in common — a folder and a table of mappings for Log, one choice for
 * Message, an address, a type, a file list and a tail for SOAP — so a flat record would be a
 * widening list of unrelated scalars whose only clue to what belongs where is a name prefix. The
 * nesting mirrors the file's key namespaces ({@code log.*}, {@code message.*}, {@code soap.*}) and
 * the Preferences tabs, so all three can be read against each other. The global settings need no
 * such grouping: being unprefixed <em>is</em> what says they belong to no mode, which was already
 * true of {@code mode} and {@code theme} before there were any others.
 *
 * <p><strong>What is deliberately not here.</strong> Nothing, now. The content opacity used to be
 * excluded as a view control rather than a setting; it was replaced in the ribbon by the three
 * global settings below, which are settings and are persisted like everything else.
 *
 ...
 * mutable {@code List} is not immutable, whatever its accessors say. {@link LogSettings} and
 * {@link SoapSettings} enforce that in their constructors rather than trusting callers.
 *
 ...
 * @param blastPort     the port data is blasted to,
...
public record Settings(Mode mode,
     ...
     *
     * <p>Deliberately not 80, 443 or 8080: an out-of-box value that is already claimed on a typical
     * machine makes the first thing anyone tries a conflict. Nothing binds it yet, so this is a
     ...
     *
     * <p>Loopback rather than a routable address. The mode has no behaviour, so the default must be
     * the one that cannot reach anything if that changes before anyone revisits this line.
     */
     ...
     *
     * <p>The address is validated for form but never checked for reachability, and the data files
     * are not checked for existence — both are connect-time and read-time concerns, and the mode
     * behaviour that would raise them does not exist yet. Discarding someone's choice because a
     * network drive happened to be offline at start-up is a worse outcome than showing a path that
     * is not currently reachable, which is the same rule the log folder follows.
     *
     * @param ip            the address SOAP mode will send to, an IPv4 dotted quad
     * @param type          which kind of message the mode will send — its own enum, not
     *                      {@link MessageType}
     ...
     */
    public record SoapSettings(String ip,
                               SoapMessageType type,
     ...
     * type, the SOAP settings and the mappings were all deliberately absent from that record, and
    ...
    public static Settings from(AppState appState) {
        ...
        List<String> dataFilePaths = appState.soapDataFiles().stream()
                .map(File::getAbsolutePath)
                .toList();
        return new Settings(
                appState.getCurrentMode(),
                appState.getTheme(),
                appState.getBlastPort(),
                appState.isSingleMessage(),
                appState.isByteHijack(),
                new LogSettings(
                ...
                new SoapSettings(
                        appState.getSoapIp(),
                        appState.getSoapMessageType(),
                        dataFilePaths,
                        appState.getSoapTail()));
    }
}
