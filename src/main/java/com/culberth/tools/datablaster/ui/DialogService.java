...
/**
 * Builds and shows the application's modal dialogs: owner, modality, stylesheet and Esc-to-cancel are applied in one
...
public class DialogService
{
    ...
    /**
     * Loads {@code fxmlResource} and shows it as an application-modal dialog, blocking until it closes.
     *
     * <p>
     * Everything from the load through {@code showAndWait} is guarded: an exception escaping here would reach the FX
     * default handler, which reports to a console the windowed build does not have, leaving the menu item looking as
     ...
     *
     * <p>
     * Here rather than at the call site for the same reason {@link #showModal} is: owner and stylesheet are applied in
     * one place. An {@code Alert} built inline gets neither, so it opens unowned and in stock light chrome — which is
     * most obvious, and most wrong, under the dark theme.
     *
     * <p>
     * {@code confirmLabel} names the action rather than saying "OK". A button that says what it does is the difference
     * between reading a dialog and dismissing it.
     *
     * <p>
     * Returns {@code false} if the dialog cannot be shown at all. A confirmation that fails open would let the very
     * action it guards proceed unattended.
     */
    ...
    /**
     * Reports a failure to the user, and logs it. The log matters: the dialog carries only the message, so without this
     * the stack trace would be discarded entirely and the console build would be harder to diagnose than one with no
