...
/**
 * Swaps the content view inside a host pane. Which mode is current is recorded in {@code AppState} by whoever requests
 * the change; this class only performs the swap.
 *
 * <p>
 * This is a stateless singleton on purpose: the host pane is passed in per call rather than retained. Holding it in a
 * field would put a node owned by a prototype, FXML-created controller inside a singleton, so a second shell would
 * silently take over the first shell's content pane.
 */
@Component
