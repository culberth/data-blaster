...
/**
 * Starts the JavaFX toolkit on Monocle's headless Glass platform, so tests that build a real scene graph need no
 * display.
 *
 * <p>
 * <strong>Why this exists.</strong> Most of this project's tests deliberately avoid the toolkit entirely — see
 * {@code AppStateTest}, which records the FX thread rather than probing it for exactly that reason. But the FXML wiring
 * cannot be verified without instantiating controls, so {@code FxmlSmokeTest} needs a toolkit. Monocle keeps that from
 * costing a display, which is what lets CI's Linux runner prove the suite is headless rather than the developer
 * asserting it.
 *
 * <p>
 * <strong>Only tests that build a scene graph may use this class.</strong> Calling {@link #start()} from a test that
 * does not need one silently widens the toolkit's footprint and re-introduces the coupling the rest of the suite
 * avoids. Public only so those tests can live in the package they belong to rather than being dragged into this one.
 */
public final class HeadlessToolkit
{
    ...
    /**
     * Initialises the toolkit if it is not already running. Idempotent, so every test class that needs a scene graph
     * can call it from {@code @BeforeAll} without coordinating with the others.
     */
    ...
    /**
     * Runs {@code action} on the JavaFX Application Thread and waits for it, re-throwing whatever it threw.
     *
     * <p>
     * Failures are carried back rather than left to the FX thread's default handler, which would print a stack trace
     * and let the test pass — the precise failure mode these tests exist to catch.
     */
